...
//Se colocar false é a mesma coisa que aperta space para mostrar todos
...
void draw() {
  ...
  //Apenas evoluindo normalmente
    ...
    if (!pop.done()) {
      pop.updateAlive(); //Atualiza quem ta vivo
    
    //all dead
    //Quando todos morrerem
    } else {
      //Calculo do AG para definir a nova geracao
      pop.calculateFitness(); //Calculo de fitness 
      pop.naturalSelection(); //Selecao natural
    }
  }
  //display the score
...
//Comandos do teclado para o jogador manipular a nave ou opcoes
...
void showScore() {
  ...
  //Mostrando o score de cada modelo
  if (humanPlaying) { //Player humano
    text("Score: " + humanPlayer.score, 80, 60);
  } else
    if (runBest) { //So mostre o melher
      text("Score: " + pop.bestPlayer.score, 80, 60);
      text("Gen: " + pop.gen, width-200, 60);
    } else {
      if (showBest) { //Mostre o score do jogador 0
